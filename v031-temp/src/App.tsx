...
export default function App() {
  ...
  const addToolMessage = (toolName: string, input: any, output: any) => {
    const msg: Message = {
      id: `msg_${Date.now()}`,
      role: 'tool',
...
function ChatTab({ 
  ...
  const runToolInChat = async (tool: Tool, toolInput: any) => {
    const toolMsg: Message = {
      id: `msg_${Date.now()}`,
      role: 'system',
      content: `Running tool: ${tool.name}...`,
      timestamp: new Date(),
    };
    ...
    try {
      ...
      onToolResult(tool.name, toolInput, result);
      
      // Remove the "running" message
      setHistory(prev => prev.filter(m => m.id !== toolMsg.id));
    } catch (e: any) {
      setHistory(prev => prev.map(m => 
        m.id === toolMsg.id ? { ...m, content: `Tool error: ${e.message}` } : m
      ));
    }
  };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {showToolPicker && filteredTools.length > 0 && (
        <div style={{
          position: 'absolute',
          bottom: 120,
          left: 16,
          ...
          {filteredTools.slice(0, 10).map(tool => (
            ...
            >
              ...
              {tool.description && (
                <div style={{ fontSize: 12, color: colors.textMuted }}>{tool.description}</div>
              )}
            </div>
          ))}
        </div>
      ...
      <div style={{ padding: 16, borderTop: `1px solid ${colors.border}`, background: colors.bgSecondary }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
          <select 
            ...
            style={{ ...styles.input, width: 150, padding: '6px 10px' }}
          >
