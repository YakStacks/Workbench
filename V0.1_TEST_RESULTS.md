...
- `App.tsx` lines 353-363: User-friendly error transformation
  - "No model configured" → Setup instructions
  - "No API key" → Key required message
  - Status codes → Readable messages
- `main.ts` line 941: Catch blocks wrap errors in standard format
- No raw stack traces exposed to UI

**Test:** 
- [ ] Run tool with invalid parameters → Shows friendly error
- [ ] Run tool with missing API key → Shows setup hint
...
## ⚠️ 2) Tool Chain Proof

### One working chain (Tool A → Tool B → final)
**Status:** ⚠️ **NEEDS TESTING**

**Available:** Chains tab exists in UI

**Test scenario:**
- [ ] Create chain: `weather.temperature` (city) → `echo` (result)
...
**Test scenario:**
- [ ] Create chain: `weather.temperature` (invalid city) → `echo`
...
**Evidence:**
- ✅ File exists: `PLUGIN_API.md`
- ✅ Contains minimal tool definition example
- ✅ Documents input schema expectation
- ✅ Specifies output contract: `{ content, metadata?, error? }`
- ✅ Explains where plugins live (`plugins/` folder)
- ✅ Describes how plugins are loaded (auto-discovery)
- ✅ Includes auto-normalization documentation
...
**Status:** ⚠️ **NEEDS IMPLEMENTATION CHECK**
...
**Evidence:**
- ✅ What it is: "A local-first AI task runner..." (line 3)
- ✅ Quickstart: Lines 120-139 (dev mode + production)
- ✅ How to install plugins: Lines 106-118 ("Build Your First Custom Tool")
- ✅ "Not a SaaS / local-first" note: Lines 6-8 ("100% Local-First")
...
**Status:** ⚠️ **NEEDS VISUAL INSPECTION**
...
### ✅ Ready for Release (No Action Needed)
...
### ⚠️ Needs Manual Testing (Before Release)
...
4. **Fresh Install** - Test `git clone → npm install → npm run dev` on clean machine
...
### 📝 Recommended Improvements (Post-v0.1.0)
...
- [ ] Try invalid city → check friendly error
...
- [ ] Run one MCP tool
- [ ] Disconnect server → verify error message
